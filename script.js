...
function time() {
  ...
  const hoursDeg = (hour * 360) / 12 + (mint * 360) / 60 / 12;
  const minutesDeg = (mint * 360) / 60 + (sec * 360) / 60 / 60;
  const secondsDeg = (sec * 360) / 60;
  document.querySelector(".hour").style.transform =
    "rotate(" + hoursDeg + "deg)";
  document.querySelector(".minute").style.transform =
    "rotate(" + minutesDeg + "deg)";
  document.querySelector(".seconds").style.transform =
